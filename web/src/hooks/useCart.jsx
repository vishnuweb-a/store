...
export const CartProvider = ({ children }) => {
  ...
  const addToCart = useCallback((product, variant, quantity, availableQuantity) => {
    return new Promise((resolve, reject) => {
      // Product grids read inventory from the list endpoint, which omits
      // inventory_quantity, so availableQuantity can be null there. Skip the
      // check when the stock level is unknown rather than rejecting the add —
      // Hostinger's checkout remains the authoritative stock gate.
      if (variant.manage_inventory && Number.isFinite(availableQuantity)) {
